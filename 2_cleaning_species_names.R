...
TPL.output <- TPL(full.list$genus_species)

clean.list <- TPL.output %>% 
  rename(old_name=Taxon) %>% 
  unite(col=new_name, c(New.Genus, New.Species), sep=' ') %>% 
  select(Plant.Name.Index, old_name, new_name) %>% 
  #manually cleaning names that were not recognized in World Flora
  mutate(new_name=ifelse(old_name=='Lespedeza bicolor for. alba', 'Lespedeza bicolor', new_name),
         Plant.Name.Index=ifelse(old_name %in% c('Acacia mangium x auriculiformis','Acacia tortillis spp. heteracantha','Lespedeza maximowiezzi var. tomentella'), 'TRUE', Plant.Name.Index))


#### write data to file ####

write.csv(clean.list, "legume_clean_names.csv")
