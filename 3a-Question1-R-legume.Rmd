...
Native_NonNative_Fig1 <- Plant_Data_Clean %>%
  ...
  select(paper_id,new_name,paper_plant_status,annual_perennial,growth_form,sample_country,sample_continent,num_nodules,strain_richness,compares_natinv) %>% 
...
#Data normality
Native_NonNative_Norm <- lm(data = Native_NonNative_Fig1, strain_richness ~ paper_plant_status)
...
Native_NonNative_Fig1<-Native_NonNative_Fig1 %>% 
  mutate(strain_richness_TF=log(strain_richness))
...
ols_test_normality(Native_NonNative_Norm_TF) #better but not normal

#lmer
Native_NonNative_lmer_2 <- lmerTest::lmer(data = Native_NonNative_Fig1, strain_richness_TF ~ paper_plant_status + (1|paper_id) + (1|new_name))
