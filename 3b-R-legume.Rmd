...
Plant_Associations <- Plant_Associations%>% left_join(Clean_Species) %>% filter(cultivation.status!="row crop", genus_species!="999", presence_absence=="1", plant_status!="999")
...
Plant_Associations_num <- Plant_Associations %>% mutate(ifelse(num_nodules=="999", NA, num_nodules))

native_nonnative.strains <- Plant_Associations_num %>% group_by(genus_species,plant_status,rhizobia_sp) %>% summarise(presence=max(presence_absence), total_nodules=sum(num_nodules)) %>% ungroup()
...
## Determine number plant species (696), clean names merged and now we have 650
Plant_Associations%>%
  group_by(new_name)%>%
  unique()%>%
...
## Determine number of countries assessed (85), now it is 78
Plant_Associations%>%
...
# Make a graph for non-native vs native, also filtering 999
