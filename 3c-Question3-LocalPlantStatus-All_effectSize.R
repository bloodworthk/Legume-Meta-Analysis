...
Native_NonNative_Fig1_lnRR <- Native_NonNative_Fig1 %>% 
  group_by(paper_id, paper_plant_status) %>% 
  summarize(strain_richness_mean=mean(avg_strain_richness)) %>% 
  ungroup() %>% 
  pivot_wider(names_from=paper_plant_status, values_from=strain_richness_mean) %>% 
  mutate(lnRR=log(nonnative/native))


hist(Native_NonNative_Fig1_lnRR$lnRR)

t.test(Native_NonNative_Fig1_lnRR$lnRR, y = NULL,
       alternative = "two.sided",
       mu = 0, paired = FALSE, var.equal = FALSE,
       conf.level = 0.95)

figDataTest <- Native_NonNative_Fig1_lnRR %>% 
  summarise(lnRR_mean=mean(lnRR, na.rm=T),
            lnRR_n=length(lnRR),
            lnRR_sd=sd(lnRR, na.rm=T)) %>% 
  mutate(strain_richness_lnRR_se=lnRR_sd/sqrt(lnRR_n),
         strain_richness_lnRR_ci=1.96*(strain_richness_lnRR_se))

ggplot(figDataTest, aes(x=lnRR_mean, y=lnRR_mean)) +
  geom_point(size = 5) +
  geom_errorbar(aes(ymin=lnRR_mean-strain_richness_lnRR_ci, ymax=lnRR_mean+strain_richness_lnRR_ci), width=0.1) +
